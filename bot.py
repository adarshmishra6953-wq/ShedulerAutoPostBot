...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "main_menu":
            ...
            text = "Aapki Scheduled Posts:"
            if not posts: text = "Koi post scheduled nahi hai."

            # Fix for "There is no text in message to edit"
            try:
...
async def handle_inputs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "waiting_for_channel":
        try:
            ...
            await update.message.reply_text("❌ Error! Username galat hai ya bot admin nahi hai.")
        ...
        try:
            ...
            await update.message.reply_text("❌ Format HH:MM use karein.")
        context.user_data.clear()
...
async def send_posts(app):
    ...
    for cid, pid, cap in cur.fetchall():
        try: 
            # Caption ko bold karne ke liye * use kiya gaya hai
            bold_caption = f"*{cap}*" if cap else ""
            ...
            print(f"✅ Success: Posted to {cid} at {now}")
        except Exception as e: 
